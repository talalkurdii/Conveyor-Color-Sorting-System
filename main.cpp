...
/*
  Colour sorter controller
  ------------------------
  This is the version I normally keep on the robot after calibration testing.
  A few of these timing values are still very machine-specific, so don't trust
  them blindly if the belt speed, lighting, or pusher travel changes.
*/

// TCS3200 colour sensor pins. Yep, these are all up on the Mega header.
const int S0_PIN = 53;
const int S1_PIN = 52;
const int S2_PIN = 51;
const int S3_PIN = 50;
...
// Motor driver 1: red/yellow pushers
const int AIN1 = 22;
const int AIN2 = 23;
const int PWMA = 2;
...
// Motor driver 2: blue pusher + belt
const int CIN1 = 27;
const int CIN2 = 28;
const int PWMC = 4;
...
// I ignore the first sample because the object/sensor reading often wobbles a bit.
...
// Sensor-to-pusher travel times. These are the first things to tweak when sorting misses.
...
// These are a bit of a hack, but red was the colour that gave me the most false misses.
...
// Reads one selected sensor channel. The result is a frequency-ish value, not a true RGB value.
